...
freq_bands.Endothelial = struct( ...
    ...
    'physio_bounds', [30, 85; 40, 95; 50, 150]); % [SE_min, SE_max; rSO2_min, rSO2_max; MAP_min, MAP_max]
...
for band_idx = 1:length(band_names)
    ...
    for p = 1:n_patients
        ...
        try
            ...
            % 6. Sáv-specifikus high-pass szűrés
            ...
            if length(SE_clean) > 10 && cutoff_freq < band_params.target_fs/2
                ...
                MAP_clean = highpass(MAP_clean, cutoff_freq, band_params.target_fs);
            end
            
            % 7. Fiziológiai határok ellenőrzése
            physio_bounds = band_params.physio_bounds;
            SE_valid = SE_clean >= physio_bounds(1,1) & SE_clean <= physio_bounds(1,2);
            rSO2_valid = rSO2_clean >= physio_bounds(2,1) & rSO2_clean <= physio_bounds(2,2);
            MAP_valid = MAP_clean >= physio_bounds(3,1) & MAP_clean <= physio_bounds(3,2);
